...
func TestSwapCheck(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			getSwapMemory = func(ctx context.Context) (*mem.SwapMemoryStat, error) {
				if tt.mockErr != nil {
					return nil, tt.mockErr
				}
				return &mem.SwapMemoryStat{UsedPercent: tt.mockPercent}, nil
			}

			chk := Swap(tt.warn, tt.fail)
